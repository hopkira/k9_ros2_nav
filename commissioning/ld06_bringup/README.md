...
## 20 cm self-return filter

The main launch now also publishes `/scan`, with returns below 0.20 m replaced
by NaN. `/scan_raw` is preserved. The output range_min is at least 0.20 m;
...
8475, with output in `~/k9_ws/ld06_bringup/filter.log`. Stop that instance before
